...
export INSTANCE_NAME="cs285-hw5"
...
echo "Syncing ./exploration_visualization ..."
rsync -av --progress -e "ssh -o StrictHostKeyChecking=no -i ~/.ssh/google_compute_engine" $IP:~/exploration_visualization ./exploration_visualization

echo "Syncing ./datasets ..."
rsync -av --progress -e "ssh -o StrictHostKeyChecking=no -i ~/.ssh/google_compute_engine" $IP:~/datasets ./datasets
